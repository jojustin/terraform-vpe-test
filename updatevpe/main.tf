...
resource "ibm_is_subnet_reserved_ip" "reservedip" {
  depends_on = [ ibm_is_subnet.subnet1 ]
  subnet = ibm_is_subnet.subnet1.id
}

resource "ibm_is_virtual_endpoint_gateway_ip" "vpeip_new" {
  depends_on = [ ibm_is_virtual_endpoint_gateway.endpoint_gateways ]
  gateway     = ibm_is_virtual_endpoint_gateway.endpoint_gateways.id
  reserved_ip = ibm_is_subnet_reserved_ip.reservedip.id
}
